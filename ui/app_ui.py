"""
Trusty Chapter 13 -- Thin-client Gradio UI.

Runs as a separate process from the FastAPI server:
    Terminal 1: uvicorn app:app --reload
    Terminal 2: python ui/app_ui.py

Talks to the backend exclusively over HTTP -- no direct imports from
app.py or any backend module. The UI is a client, not a plugin.
"""
...
def upload_file(file_path: str, session_id: str) -> dict:
    """Upload a file to /upload and return the parsed response."""
...
def upload_url(url: str, session_id: str) -> dict:
    """Upload a URL to /upload and return the parsed response."""
    response = requests.post(
        f"{API_BASE}/upload",
        data={"url": url},
        headers={"X-Session-ID": session_id},
...
def ask_question(question: str, session_id: str) -> dict:
    """POST a question to /ask and return the parsed response."""
    response = requests.post(
        f"{API_BASE}/ask",
        json={"text": question},
        headers={"X-Session-ID": session_id},
...
def format_verification(parsed_report: dict) -> str:
    """Format the verification report as a readable status block."""
...
def format_budget(budget: dict) -> str:
    """Format the budget summary as a compact status line."""
    if not budget:
        return ""
...
def format_trace(trace: dict) -> str:
    """Format the decision trace as a readable timeline."""
    if not trace:
        return ""
...
def handle_new_session():
    """Mint a fresh session ID -- wipes all state for this browser tab."""
...
def handle_upload(file, url_input, session_state):
    ...
    try:
        if file is not None:
            ...
            msg = f"[OK] **{filename}** indexed -- {added} chunks added ({total} total this session)"
            if skipped:
                msg += f"\n[WARNING] Skipped inside archive: {', '.join(skipped)}"
        else:
...
def handle_ask(question, session_state):
    """Handle a question -- returns (answer, verification, budget, trace, error)."""
    session_id = session_state.get("session_id")

    if not session_id:
        return "", "", "", "", "[WARNING] No session -- click 'New Session' first."
    if not question.strip():
        return "", "", "", "", "[WARNING] Please enter a question."
    if session_state.get("uploads", 0) == 0:
        return "", "", "", "", "[WARNING] No documents uploaded yet. Upload a file or URL first."

    try:
        result = ask_question(question.strip(), session_id)
        ...
        return answer, verification, budget_str, trace_str, ""
        ...
        return "", "", "", "", f"[ERROR] Ask failed: {detail}"
    except Exception as e:
        return "", "", "", "", f"[ERROR] {e}"
...
def build_ui():
    with gr.Blocks(title="Trusty", theme=gr.themes.Soft()) as demo:
        ...
        # Per-tab session state -- invisible across browser tabs
